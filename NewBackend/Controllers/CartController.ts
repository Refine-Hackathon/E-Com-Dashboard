...
export const addToCart = (req: Request, res: Response) => {
    const query1 = 'SELECT * from `cart` where user_id = "'+req.body.uid+'" and prdt_id = "'+req.body.pid+'" and size = "'+req.body.size +'"';
...
export const viewCart = (req: Request, res: Response) => {
    const query1 = 'SELECT c.cart_id , c.size , c.count ,p.*  from `cart` c , `products` p  WHERE c.user_id = "'+req.params.uid+'" and c.prdt_id = p.prdt_id';
        const query2 = 'SELECT  sum(p.product_cost) as total from `cart` c , `products` p  WHERE c.user_id = "'+req.params.uid+'" and c.prdt_id = p.prdt_id';
        db.query(query1 , (err , result : Array<any>) => {
            if(err){ 
                console.log(err);
                return res.status(500).json({msg:err});}
